...
<div class="relative h-full">
    ...
    <div class="absolute bottom-0 left-0 right-0 z-10">
        <Collapsible.Root bind:open={telemetryOpen}>
            <div
                class={`backdrop-blur border-t ${connectionStore.connected ? "bg-background/95" : "bg-destructive/95"}`}
            >
                <!-- Expanded View -->
                <Collapsible.Content>
                ...
                <button
                    ...
                    class={`w-full px-4 py-2 flex items-center justify-between transition-colors ${connectionStore.connected ? "hover:bg-accent/50" : "hover:bg-destructive/80"}`}
                >
                    <div
                        class={`flex items-center gap-6 text-sm ${connectionStore.connected ? "" : "text-destructive-foreground"}`}
                    >
                    ...
                    <ChevronDown
                        class={`h-4 w-4 transition-transform duration-200 ${telemetryOpen ? "rotate-180" : ""}`}
                    />
                </button>
            </div>
        </Collapsible.Root>
