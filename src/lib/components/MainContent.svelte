<script lang="ts">
  ...
  function removeTrajectory(trajectory: Trajectory) {
    savedTrajectories = savedTrajectories.filter(t => t.id !== trajectory.id);
  }
...
<div class="h-full flex flex-col">
  ...
  <div class="flex-1">
    ...
    {:else}
      ...
      <Tabs.Root value={savedTrajectories[0]?.id} class="h-full flex flex-col">
        <Tabs.List class="border-b px-4">
          {#each savedTrajectories as trajectory}
            <Tabs.Trigger value={trajectory.id}>
              {trajectory.name}
              <Button size="icon" variant="ghost" onclick={() => removeTrajectory(trajectory)} class="rounded-full"><X class="text-teal-500"/></Button>
            </Tabs.Trigger>
          {/each}
          <Button variant="ghost" size="sm" class="ml-2" onclick={loadTrajectory}>
            + Load Trajectory
          </Button>
        </Tabs.List>
        ...
        {#each savedTrajectories as trajectory}
          <Tabs.Content value={trajectory.id} class="flex-1">
            <TrajectoryPlot x={trajectory.x} y={trajectory.y} z={trajectory.z} />
          </Tabs.Content>
        {/each}
      </Tabs.Root>
    {/if}
